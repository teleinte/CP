	...
	$("#evento_crear_form").submit(function(event){
		...
		else
		{
	        ...
	        if(response)
	        {
    			...
    			/*var link = btoa(encodeURIComponent(JSON.stringify({description:$('#notase').val(),location:sessionStorage.getItem('ncp'),end:$('#datepicker4').val().replace('-','') + "T" + $('#endtimee').val().replace(":","") + "00Z",start:$('#datepicker3').val().replace('-','') + "T" + $('#starttimee').val().replace(":","") + "00Z"})));
    			var body = '<h4 style="color:#666 !important;">'+ sessionStorage.getItem('nombreCompleto') + obtenerTerminoLenguage('ma','1') + sessionStorage.getItem('ncp')+ obtenerTerminoLenguage('ma','30') +$('#nombree').val() +'.</h4><ul style="color:#666 !important;"><li>'+obtenerTerminoLenguage('ma','3')+$('#datepicker3').val()+'</li><li>'+obtenerTerminoLenguage('ma','24')+$('#datepicker4').val()+'</li><li>'+obtenerTerminoLenguage('ma','4')+$('#starttimee').val()+'</li><li>'+obtenerTerminoLenguage('ma','25')+$('#endtimee').val()+'</li></ul></p><p><a href="http://www.google.com/calendar/event?action=TEMPLATE&text=' + encodeURIComponent($('#nombree').val()) + '&details=' + encodeURIComponent($('#notase').val()) + '&dates=' + $('#datepicker3').val().replace('-','').replace('-','') + 'T' + $('#starttimee').val().replace(":","") + '00/' + $('#datepicker4').val().replace('-','').replace('-','') + 'T' + $('#endtimee').val().replace(":","") + '00&location=' + encodeURIComponent(sessionStorage.getItem('ncp')) + '" style="background:#f51e7c; color:#fff!important; text-decoration: none; margin-top:10px; padding:5px 10px; border-radius: 3px; font-weight:bold;">Agregar evento a Google Calendar</a>&nbsp;&nbsp;<a href="' + traerDireccion() + 'api/services/event.php?token=' + link + '" style="background:#f51e7c; color:#fff!important; text-decoration: none; margin-top:10px; padding:5px 10px; border-radius: 3px; font-weight:bold;">Descargar archivo .ics (iCalendar, Outlook)</a></p>';*/
    			$(document).renderme('ma');
