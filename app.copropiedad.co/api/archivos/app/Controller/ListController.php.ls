	...
	try
	{
		...
		if($tokenValidado)
		{
			...
			if ($validateUsuario)
			{
				...
				{
					case 'archive_item':
						enviarInformacion('task', $datos->body, $app);
						break;					
					default:					
...
$app->get("/list/:buildingId/:lista", function($buildingId,$lista) use($app)
{
	try
	{
		$validateUsuario=validateRole();
		if ($validateUsuario)
		{
			...
			{
				case 'archive_item':
					consultaColeccion($app, 'task', array('building_id' => (float)$buildingId,'status' => 1));
					break;				
			}
		}
		else{enviarRespuesta($app, false, "Usuario sin privilegios", "Usuario sin privilegios");}
			...
			if ($validateUsuario)
			{
				switch ($datos->body->item_type)
				{
					...
					case 'queue_item':
						...
						unset($datos->body->id);
						$muestreo=array("_id"=>new MongoId($idGuardado));
						$dbdata=new DBNosql('queue');
						$array = json_decode(json_encode($datos), true);						
						$result=$dbdata->updateDocument($muestreo, $datos->body);
						if ($result){enviarRespuesta($app, true, $result, "null");}
						else {enviarRespuesta($app, true, null, null);}
						break;
					case 'auth_board_item':
						$idGuardado=$datos->body->id;
						unset($datos->body->id);
						$muestreo=array("_id"=>new MongoId($idGuardado));
						$dbdata=new DBNosql('auth_board');
						$array = json_decode(json_encode($datos), true);						
						$result=$dbdata->updateDocument($muestreo, $datos->body);
						if ($result){enviarRespuesta($app, true, $result, "null");}
						else {enviarRespuesta($app, true, null, null);}
