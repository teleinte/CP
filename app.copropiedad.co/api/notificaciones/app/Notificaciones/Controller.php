<?php
require_once("/datos/app.copropiedad.co/api/notificaciones/app/Model/config.php");
require_once("/datos/app.copropiedad.co/api/notificaciones/app/Model/Token_model.php");
require_once("/datos/app.copropiedad.co/api/notificaciones/app/Model/Log_model.php");
require_once("/datos/app.copropiedad.co/api/notificaciones/app/Model/DBNosql_model.php");
...
$app->options("/notificaciones/", function() use($app)
{
  enviarRespuesta($app, true, "ok", "ok");
});
...
//MÉTODO MODIFICAR NOTIFICACION - OK
  $app->put("/notificaciones/", function() use($app)
  {
   try
    {
      $requerimiento = $app::getInstance()->request();
...
$app->options("/notificaciones/usuario/", function() use($app)
{
  enviarRespuesta($app, true, "ok", "ok");
});
...
$app->options("/notificaciones/todos/", function() use($app)
{
  enviarRespuesta($app, true, "ok", "ok");
});
   ...
   try
    {
      ...
      if($token->SetToken($datos->token))
      {
        ...
        if ($validateUsuario)
        {
          ...
          $res = consultaColeccionRespuesta($app, 'notificaciones', array());//$filtro);
          enviarRespuesta($app, true, $res, "null");
        }
        else
        {
...
//METODO LISTAR UNA NOTIFICACION ESPECIFICA EN EDITAR - OK - OK
  $app->options("/notificaciones/edit", function() use($app)
  {
    enviarRespuesta($app, true, "ok", "ok");
  });
  $app->post("/notificaciones/edit", function() use($app)
  {
   try
    {
      $requerimiento = $app::getInstance()->request();
      ...
      if($token->SetToken($datos->token))
      {
        ...
        if ($validateUsuario)
        {
          $idGuardado=$datos->body->id;
          $muestreo=array("_id"=>new MongoId($idGuardado));
          consultaColeccion($app, 'notificaciones', $muestreo);
        }
        else
        {
          enviarRespuesta($app, false, "Usuario sin privilegios", "Usuario sin privilegios");
