	...
	try
	{
		...
		if($tokenValidado)
		{
			...
			if ($validateUsuario)
			{
				...
				$datos->body->estado = "abierta"; 
				switch ($datos->body->item_type)
				{
					case 'solicitudes':
						enviarInformacion('solicitudes', $datos->body, $app);
						break;					
					default:					
...
$app->options("/getlist/abierta", function() use($app)
{
	enviarRespuesta($app, true, "ok", "ok");
});

$app->post("/getlist/abierta", function() use($app)
{
	// try
	// {
		$requerimiento = $app::getInstance()->request();
		...
		if($tokenValidado)
		{
			...
			if ($validateUsuario)
			{
				...
				{
					case 'solicitudes':					
					    consultaColeccion($app, 'solicitudes', array('id_copropiedad' =>$datos->body->id_copropiedad, 'estado'=>"abierta"));
						break;					
					default:					
						enviarRespuesta($app, false, "Lista Inexistente", "Lista Inexistente");
						break;
...
$app->options("/getlist/completada", function() use($app)
{
	enviarRespuesta($app, true, "ok", "ok");
});

$app->post("/getlist/completada", function() use($app)
{
	// try
	// {
		$requerimiento = $app::getInstance()->request();
		...
		if($tokenValidado)
		{
			...
			if ($validateUsuario)
			{
				...
				{
					case 'solicitudes':					
					    consultaColeccion($app, 'solicitudes', array('id_copropiedad' =>$datos->body->id_copropiedad, 'estado'=>"completada"));
						break;					
					default:					
						enviarRespuesta($app, false, "Lista Inexistente", "Lista Inexistente");
						break;
