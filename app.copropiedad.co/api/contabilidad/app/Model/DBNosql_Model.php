...
Class DBNosql
{
    ...
    {
        try {
            $this->ocon = new MongoClient("mongodb://".__DBHOST.":".__DBPORT,['username' => __DBUSERNAME, 'password' => __DBPASSWORD, 'db' => __DBNAME]);
            if (__DEBUG)
            {
                $date=date('YmdHis');
    ...
    public function removeDocumentByID($id)
    {
        try
        {
            if (__DEBUG)
            {
                $date=date('YmdHis');
                //$fix=implode(",", $fields);
                $objs = array('date'=>$date,'type'=>'Mensaje','sender'=>$this->collectionSet, 'message'=>$id, 'request'=>'remove', 'response'=>'');
                Log::Insertlog("db","FILELOG",$objs);
            }             
            ...
            if (__DEBUG)
            {
                $date=date('YmdHis');
                //$fix=implode(",", $fields);
                $objs = array('date'=>$date,'type'=>'Mensaje','sender'=>$this->collectionSet, 'message'=>$id, 'request'=>'', 'response'=>'Okay');
                Log::Insertlog("db","FILELOG",$objs);
            } 
            return  "Okay";
        } 
        catch (MongoException $e) 
        {
            $date=date('YmdHis');
            //var_dump($e);
            $objs = array('date'=>$date,'type'=>'Error','sender'=>$this->collectionSet, 'message'=>$id, 'request'=>'Actualizarndo Documentos', 'response'=>"No puede actualizar los datos");
            Log::Insertlog("db","FILELOG",$objs);
            return "Can't remove";
        }
    }
