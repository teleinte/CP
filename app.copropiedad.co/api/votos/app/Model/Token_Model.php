...
require_once('/datos/app.copropiedad.co/api/votos/app/Model/config.php');
require_once('/datos/app.copropiedad.co/api/votos/app/Model/Log_Model.php');
...
class Token
{
	...
	public function GetTokenEncuestas($fields=array())
	{
		if (__DEBUG)
        {
            $date=date('YmdHis');
        ...
        $objs = array('date'=>$date,'type'=>'Message','sender'=>"remote sender", 'message'=>'Solicitud de token', 'request'=>'GetToken'.$campos, 'response'=>'null');
        Log::Insertlog("tokens","DBLOG",$objs);
        //Finaliza el log de eventos


    	$this->fields=$fields;
    	

		$fechaInicial 	= date('YmdHis', (strtotime ($this->fields['fechaFin'])));		
		$this->fields['fechaFin']=$fechaInicial;
		//array_push($this->fields, $fechaInicial);
		//pasar de arreglo a string
		$separated = implode("|", $this->fields);
		//Proceso para generar el token de seguridad
	...
	public function SetTokenEncuestas($token)
	{
		if (__DEBUG)
        {
            $date=date('YmdHis');            
		...
		$dateToken = end($part);
		$dateNow = date('YmdHis');
		//echo "<br>fecha del token--> ".$dateToken." fecha del sistema-->".$dateNow."<br>\n"; 
