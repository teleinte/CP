<?php
require_once("/datos/app.copropiedad.co/api/votos/app/Model/Token_Model.php");
require_once("/datos/app.copropiedad.co/api/votos/app/Model/Log_Model.php");
require_once("/datos/app.copropiedad.co/api/votos/app/Model/DBNosql_Model.php");
...
$app->post("/encuesta/copropiedad/activas/", function() use($app)
{
	try
	{
		...
		if($tokenValidado)
		{
			...
			if ($validateUsuario)
			{
				consultaColeccion($app, 'encuestaCabecera', array('id_copropiedad' => $datos->body->id_copropiedad, 'estado' => "1"));
			}
			else{enviarRespuesta($app, false, "Usuario sin privilegios", "Usuario sin privilegios");}
		}
		else{enviarRespuesta($app, false, "Token invalido", "Token invalidos");}
