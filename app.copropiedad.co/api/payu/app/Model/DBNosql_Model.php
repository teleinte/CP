...
Class DBNosql
{
    ...
    public function selectDocument($fields = array())
    {
        if (__DEBUG)
        {
            ...
            $date=date('YmdHis');
            //$fix=implode(",", $res);
            $objs = array('date'=>$date,'type'=>'Mensaje','sender'=>$this->collectionSet, 'message'=>$res, 'request'=>'Listar Documentos', 'response'=>'');
            Log::Insertlog("db","FILELOG",$objs);
            $stringDbug="";
        }    	
        ...
        if (__DEBUG)
        {
            ...
            //var_dump($envio);
            $objs = array('date'=>$date,'type'=>'Mensaje','sender'=>$this->collectionSet, 'message'=>$fields, 'request'=>'', 'response'=>$envio);
            Log::Insertlog("db","FILELOG",$objs);
        } 
    	return $this->docs;
    }
    ...
    public function updateMDocument($fields, $newdata)
    {
        try
        {
            if (__DEBUG)
            ...
            $this->collection->update($fields,$newdata ,array('multiple' => true));
            if (__DEBUG)
            {
                $date=date('YmdHis');
                $objs = array('date'=>$date,'type'=>'Mensaje - UPDATE','sender'=>$this->collectionSet, 'message'=>json_decode(json_encode($newdata), true), 'request'=>'', 'response'=>'Okay');
