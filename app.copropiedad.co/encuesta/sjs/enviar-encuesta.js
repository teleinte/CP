$(document).ready(function(){
  $("#odestinatario").attr('required',true); 

  $("#invitados").change(function()
  {
    if($("#invitados").val()=="manual")
    {
      $("#odestinatario").attr('required',true);     
    }
    else
    { 
      $("#odestinatario").attr('required',false);           
    }
  });

    $("#alertas").html('<div class="alert alert-dismissable alert-info" teid="ale:html:131"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button></div>');  
    ...
    $("#form-envio-encuesta").submit(function(event){
      ...
      {
        $(document).renderme('en');
        enviocorreoencuesta("app.copropiedad.co", correosTotales, $('#mensaje').val(), $('#fechaFin').val().split("T")[0], obtenerTerminoLenguage('ma','50') +$('#asunto').val(), traerDireccion()+"render-encuesta/",retornado)
        $(document).renderme('en');
