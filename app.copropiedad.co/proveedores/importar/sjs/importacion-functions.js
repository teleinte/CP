  ...
  reader.onload = function(event) {
    ...
    $.each(data.split(/\n?\r/),function(k,v){
      ...
      if(contador != 0 && contador != valorFinal)
      {
        ...
        var nit = v.split(separador)[5];
        if(nombre_proveedor.length > 2 && nombre_contacto.length > 2 && apellido_contacto.length > 2 && telefono.length > 2 && email.length > 2 && nit.length > 2)
        {
          $("#agregar_campos > tbody:last-child").append('<tr style="text-align:center;"><td width="15%" class="titulo-campos">'+filador+'</td><td>'+nombre_proveedor+'</td><td>'+nombre_contacto+'</td><td>'+apellido_contacto+'</td><td>'+telefono+'</td><td>'+email+'</td><td>'+nit+'</td></tr>');
            stringEnviado+=nombre_proveedor+","+nombre_contacto+","+apellido_contacto+","+telefono+","+email+",proveedor,"+nit+"|";
          filador++;
        }
        else
        {
