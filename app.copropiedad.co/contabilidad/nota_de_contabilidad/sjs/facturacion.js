$(document).ready(function() {
    ...
    $("#consecutivo").html(obtenerConsecutivos("nc",datos));
        ...
        if(option.val())
        {
          if(cta.val() || cta.val() == undefined)
          {
            if(mov.val() || mov.val() == undefined)
            {
              if(iCnt > 0)
              {
                result = crearDocumento(iCnt, $("#consecutivo").html(), $("#cpnomf").html(), $("#cpema").html(), $("#conceptodoc").val(), "", $("#docext").val(), "NC", $("#idtercero").val(),$("#idnit").val());
                if(result)
                {
        ...
        if(option.val())
        {
          if(cta.val() || cta.val() == undefined)
          {
            if(mov.val() || mov.val() == undefined)
            {
              if(iCnt > 0)
              {
                result = crearDocumento(iCnt, $("#consecutivo").html(), $("#cpnomf").html(), $("#cpema").html(), $("#conceptodoc").val(), "", $("#docext").val(), "NC", $("#idtercero").val());
