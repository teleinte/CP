...
{  
  $(document).renderme('co');
  	$.each(datos, function(x , y) 
    {
        ...
        $("div.toolbar").html('<input type="button" class="btn ttip positivo" id="envio_caso" teid="co:val:283"/>');
        $.each(y, function(a , b)
        {
            if(a=="puc")
            {
                $.each(b, function(e , r)
                {
                    ...
                    {
                        var acciones ='<a href="editar-cuenta.php?idt='+entradaDos['cuenta']+'&base='+e+'&idm='+idMongoFinal.$id+'&n='+encodeURIComponent(entradaDos['nombre'])+'"> Cambiar nombre        |</a><a href="agregar-cuenta.php?idt='+entradaDos['cuenta']+'&base='+e+'&idm='+idMongoFinal.$id+'">Agregar sub-cuenta</a>';
                    }
                    if(entradaDos['editar'] && !entradaDos['agregar'])
                    {
