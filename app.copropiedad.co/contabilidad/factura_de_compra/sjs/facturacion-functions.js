...
function crearTransaccion(consecutivo, nombretercero, emailtercero, concepto_documento, notas_documento, documento_relacionado, tipo_transaccion, idcrmtercero)
{
  ...
  //console.warn(d, newdate, _year, _month, _day);
    ...
    $.ajax(
        {
            ...
            success: function(msg) 
            {
                ...
                if (datos=="Token invalido")
                {
                    $('#alertas').html('<div class="alert alert-dismissable alert-error"teid="ale:html:24"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><strong teid="ale:html:1"></strong></div>')
                    $(document).renderme('co');
                    window.location = '../../index.html';
                }
                else
