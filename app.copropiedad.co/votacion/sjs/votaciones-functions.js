function popularTablaVoraciones(datos)
{
	if(datos.length > 0)
		$.each(datos, function(x , y){
           ...
           if(llegada)
           {
            ...
            if(y['estado']=="2")
            {
                ...
                {
                    var enlace = '<a class="btn enviar solo inline ttip" teid="en:title:9" id="open-enviar-encuesta" href="enviar-votacion.php?idt='+idMongoFinal.$id+'"></a><a class="btn actualizar solo inline ttip" id="open-enviar-encuesta" teid="vo:title:78" href="enviar-votacion-recordatorio.php?idt='+idMongoFinal.$id+'"></a>';    
                }
                else
                {
                    var enlace = '<a class="btn resultados solo inline ttip" teid="en:title:10" href="resultado-votacion.php?idt='+idMongoFinal.$id+'"></a><a class="btn enviar solo inline ttip" teid="en:title:9" id="open-enviar-encuesta" href="enviar-votacion.php?idt='+idMongoFinal.$id+'"></a><a class="btn actualizar solo inline ttip" id="open-enviar-encuesta" teid="vo:title:78" href="enviar-votacion-recordatorio.php?idt='+idMongoFinal.$id+'"></a>';    
                }
            }
                t.row.add([
                '',                            
...
function traerPreguntas(datos)
{
    $.each(datos, function(x , y) 
    {
        ...
        {
            var tipejo= obtenerTerminoLenguage('vo','48');
        }
        if(y['tipo']==="seleccion_multiple_unica_respuesta")
        {
            var tipejo= obtenerTerminoLenguage('vo','49');
        }
        if(y['tipo']==="abierta")
        {
            var tipejo=obtenerTerminoLenguage('vo','50');
        }
        t.row.add([
        '',                            
        y['pregunta'],                                
        opciones,
        '<a class="btn editar solo inline ttip" teid="vo:title:8" href="pregunta-editar.php?idt='+idMongoFinal.$id+'"></a>'
...
function traerCabecerasRersultados(datos){
    ...
    $.each(datos, function(x , y) {               
        var idmongo= JSON.stringify(y['_id']);
        var idMongoFinal = JSON.parse(idmongo);
        $('#tituloEncuesta').html(obtenerTerminoLenguage('vo','46')+y['nombre']);
        $('#id_copropiedad').val(y['id_copropiedad']);
        $('#id_crm_persona').val(y['id_crm_persona']);
        $('#fecha_creacion').val(y['fecha_creacion']);
        $('#tipo').val(y['tipo']);
...
function traerPreguntasResultado(datos){    
    ...
    $.each(datos, function(x , y) 
    {
        ...
        muestra[1],
        y,
        '<a href="#" class="btn ver solo inline" title="Pulse para ver resultados" id="pregunta'+contador+'"></a>'
        ] ).draw();
...
function traerElectantes(datos){
    ...
    $('#totalInvitados').html(obtenerTerminoLenguage('vo','47')+" "+JSON.stringify(correosTotales.length));
    $(document).renderme('vo');
}
...
function enviocorreovotacion(actualurl, email, mensaje, fechafin, titulo, direccionEncuesta,tokenencuesta) //--OK
{
