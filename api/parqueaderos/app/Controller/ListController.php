	...
	try
	{
		...
		if($tokenValidado)
		{
			...
			if ($validateUsuario)
			{
				...
				{
					case 'parqueaderos_item':
						enviarInformacion('parqueaderos', $datos->body, $app);
						break;					
					default:					
	...
	// {
		...
		if($tokenValidado)
		{
			...
			if ($validateUsuario)
			{
				...
				{
					case 'parqueaderos_item':											    
					    consultaColeccion($app, 'parqueaderos', array('id_copropiedad' => (float)$datos->body->id_copropiedad,'status' => 1));
						break;					
					default:					
						enviarRespuesta($app, false, "Lista Inexistente", "Lista Inexistente");
						break;
			...
			if ($validateUsuario)
			{
				switch ($datos->body->item_type)
				{
					case 'parqueaderos_item':
						...
						$dbdata=new DBNosql('parqueaderos');
