<?php
require_once("/var/www/html/api/reservas/app/Model/config.php");
require_once("/var/www/html/api/reservas/app/Model/Token_model.php");
require_once("/var/www/html/api/reservas/app/Model/Log_model.php");
require_once("/var/www/html/api/reservas/app/Model/DBNosql_model.php");
   ...
   try
    {
      ...
      if($token->SetToken($datos->token))
      {
        ...
        if ($validateUsuario)
        {
          $filtro = array("fecha_inicio" => array('$lte' => $end."COT23:59:00+00:00", '$gte' => $today."COT00:00:00+00:00"), 'id_inmueble' => $id_inmueble);
          $res = consultaColeccionRespuesta($app, 'reservas', $filtro);
...
function validarInmueblesSinConfiguracion($inmueblesreservables, $inmueblesconfigurados)
{
  ...
  return array_udiff($inmueblesreservables, $resp, function ($obj_a, $obj_b) {
    return strcmp((string)$obj_a['_id'], (string)$obj_b['_id']);
  });
}
